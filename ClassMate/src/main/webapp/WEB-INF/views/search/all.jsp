...
<c:if test="${not empty list}">
	            ...
	            <th><h3 style="text-align: center;">${search }에 대한 카테고리 검색결과</h3></th>
...
<c:if test="${not empty list1}">
	<div id="defaultOnClassList">
		<c:forEach var="c" items="${list1}" varStatus="loop">
			<c:if test="${c.deleteBoolean == 0 && loop.index == 0}">
				<script>
					document.addEventListener('DOMContentLoaded', function() {
						var subCategoryNoValue = ${c.subCategoryNo};
						setTimeout(function() {
							window.location.href = '/main/onClassList?subCategoryNo=' + subCategoryNoValue;
						}, 1); // 1000밀리초(1초) 후에 페이지를 전환합니다.
					});
				</script>
			</c:if>
		</c:forEach>
...
<c:if test="${empty list and empty list1}">
	<br><br>
    <p style="font-size:40px; text-align: center;">${search }에 대한 검색결과가 없습니다.</p>
